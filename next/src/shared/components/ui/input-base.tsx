...
function InputBase({
  ...
  const controlReference = React.useRef<HTMLElement>(null);
    ...
    >
      <Primitive.div
        ...
        onClick={composeEventHandlers(onClick, (event) => {
          if (controlReference.current && event.currentTarget === event.target) {
            controlReference.current.focus();
          }
        })}
        className={cn(
...
function InputBaseControl({
  ...
  const composedReferences = useComposedRefs(controlRef, ref);
...
function InputBaseAdornmentButton({
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      disabled={disabled || disabledProperty}
